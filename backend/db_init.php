<?php
// db_init.php - Automatic Self-Healing Database Initialization for Axar Creative Management Solutions

function auto_init_database(PDO $pdo) {
    try {
        ...
        // -------------------------------------------------------------
        // AUTO-SEED: Insert default records if tables are empty
        // -------------------------------------------------------------
        ...
        // Seed Training if empty
        $trCount = $pdo->query("SELECT COUNT(*) FROM training_programs")->fetchColumn();
        if ($trCount == 0) {
            $pdo->exec("
        ...
        $resCount = $pdo->query("SELECT COUNT(*) FROM resources")->fetchColumn();
        if ($resCount == 0) {
            $pdo->exec("
